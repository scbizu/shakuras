...
type FavList struct {
	...
	Data   struct {
		Vlist []struct {
			...
			Vname string   `json:"title"`
		} `json:"vlist"`
	} `json:"data"`
}

//ReadFavList read uid from yaml
...
func analyseFavList(AID string) error {
	...
	avw := io.MultiWriter(av)
	n, err := io.Copy(avw, resp.Body)
	fmt.Println("Remote Length:", resp.ContentLength, "Local Length:", n)
	if err != nil {
		return err
	}
	return nil
...
func getFavList(UID string, BID int) ([]int, error) {
	...
	for _, v := range favlist.Data.Vlist {
		...
		if res == nil {
			...
			avmap[strconv.Itoa(v.VID)] = string(avinfo)
			err = model.PutAv2db(model.Bucketname, avmap)
			if err != nil {
				return nil, err
			}
			avs = append(avs, v.VID)
		}
...
func (fav *FavInfo) Run() error {
	...
	err := model.CreateBucket(model.Bucketname)
	if err != nil {
		return err
	}
